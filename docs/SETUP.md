# Getting Started with bot_army_auditor_measurement_executor

This guide walks you through setting up Measurement Executor Bot for local development.
...
DATABASE_NAME=bot_army_auditor_measurement_executor_dev
```
...
Measurement Executor Bot uses the **BotArmy.GenBot** harness to run skills. Skills are discrete, reusable units of work triggered via NATS messages.

### Creating Your First Skill

1. **Start with the template** — `lib/bot_army_auditor_measurement_executor/skills/example.ex` is a fully documented template
2. **Copy and rename** it for your skill:
   ```bash
   cp lib/bot_army_auditor_measurement_executor/skills/example.ex lib/bot_army_auditor_measurement_executor/skills/my_skill.ex
   ```
...
```elixir
defmodule BotArmyAuditorMeasurementExecutor.Skills.Classify do
  use BotArmy.Skill

  def name, do: :classify
  def description, do: "Classifies text into categories"
  def nats_triggers, do: ["bot.bot_army_auditor_measurement_executor.command.classify"]
...
```elixir
# In lib/bot_army_auditor_measurement_executor/application.ex or similar

defmodule BotArmyAuditorMeasurementExecutor.MyBot do
  use BotArmy.GenBot,
    skills: [
      BotArmyAuditorMeasurementExecutor.Skills.Classify,
      BotArmyAuditorMeasurementExecutor.Skills.ExtractEntities
    ],
    bot_id: :my_bot
end
```
...
```bash
# Make your changes
edit lib/bot_army_auditor_measurement_executor/skills/my_skill.ex
...
This creates a release named `measurement_executor_bot` that is deployed to `/opt/ergon/releases/measurement_executor_bot/` on the server.
...
3. Extracts and deploys to `/opt/ergon/releases/measurement_executor_bot/`
