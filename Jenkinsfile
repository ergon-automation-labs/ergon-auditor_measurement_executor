pipeline {
  ...
  environment {
    BOT_NAME = 'measurement_executor_bot'
    RELEASE_DIR = "/opt/ergon/releases/${BOT_NAME}"
    GITHUB_REPO = "ergon-automation-labs/ergon-auditor_measurement_executor"
  }
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/measurement_executor_bot/bin/measurement_executor_bot"
          ...
          echo "Running: $RELEASE_BIN eval 'BotArmyAuditorMeasurementExecutor.Release.create()'"
          $RELEASE_BIN eval 'BotArmyAuditorMeasurementExecutor.Release.create()'

          echo "Running: $RELEASE_BIN eval 'BotArmyAuditorMeasurementExecutor.Release.migrate()'"
          $RELEASE_BIN eval 'BotArmyAuditorMeasurementExecutor.Release.migrate()'
  ...
  post {
    success {
      sh '''
        ...
        if [ -f ./release-artifact/bot_army_auditor_measurement_executor/releases/start_erl.data ]; then
          VERSION=$(awk '{print $2}' ./release-artifact/bot_army_auditor_measurement_executor/releases/start_erl.data)
        fi
